...
const BUTTONS_CONTAINER_SELECTOR = "#FLOATING_TASK_BAR > div.filter-container > div.content-filter-item";
...
function getCheckboxes() {
    return [...$$("[type=checkbox]")].filter((checkbox) => checkbox.id.includes("KindleEBook"));
}
function getBooks(getAllBooks = true) {
...
function addButton(base_button, id, text, listener) {
...
async function removeExpiredBook(asin) {
    const TITLE = $(`#content-title-${asin}`);
    if (!TITLE)
        throw new Error(`No title found with ASIN: ${asin}`);
    const ROWS = TITLE.parentElement?.querySelectorAll(".information_row span");
    if (!ROWS)
        throw new Error(`No info rows found for ASIN: ${asin}`);
    const RETURN_BOOK_BUTTON = $(`#RETURN_CONTENT_ACTION_${asin}_CONFIRM > span`);
    if (ROWS[0].innerText === "This book was a Kindle digital library loan" &&
        ROWS[1].innerText === "Expired on") {
        RETURN_BOOK_BUTTON.click();
    }
    await sleep(1500);
}
async function downloadBook(asin) {
    const kindleDevice = $(`#download_and_transfer_list_${asin}_0`);
    if (!kindleDevice)
        return;
    await waitForElement(`#DOWNLOAD_AND_TRANSFER_ACTION_${asin}_CONFIRM > span`).then((obj) => {
        obj.click();
    });
